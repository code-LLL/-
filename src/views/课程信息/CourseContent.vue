    ...
    <CourseContentModel>
      ...
      <ContentList slot="content-list" width="185px">
        ...
        <ContentListItem
          ...
          v-for="(contentList,index) in contentLists"
          :key="index"
        ...
        >
          ...
          <OtherItem
            slot="other-item"
            v-for="(item,itemkey) in contentList.items"
            :key="itemkey"
      ...
      <ContentBox slot="content-box">
        ...
        <span slot="position" v-show="ifFold!=-1&&itemNameth!=-1?1:0">&gt;{{currentitem}}</span>
          ...
          <csContent slot="differ-content">
            ...
            <p
              slot="p-content"
            >操作系统是管理计算机硬件与软件资源的计算机程序，同时也是计算机系统的内核与基石。操作系统需要处理如管理与配置内存、决定系统资源供需的优先次序、控制输入设备与输出设备、操作网络与管理文件系统等基本事务。</p>
          </csContent>
      </ContentBox>
    </CourseContentModel>
  </div>
...
export default {
  ...
  computed: {
    ...
    currentitem() {
      return this.ifFold != -1 && this.itemNameth != -1
        ? this.contentLists[this.ifFold].items[this.itemNameth]
        : "";
    },
    currenttitlename() {
      if (this.itemNameth != -1)
        return this.contentLists[this.ifFold].items[this.itemNameth];
